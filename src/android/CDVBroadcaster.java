...
public class CDVBroadcaster extends CordovaPlugin {
     ...
     * @param filter
     */
    protected void registerExternalIntentReceiver(android.content.BroadcastReceiver receiver, android.content.IntentFilter filter) {
        this.webView.getContext().registerReceiver(receiver, filter);
    }
    ...
    private void sendGlobalBroadcast(final String action, JSONObject extras, int flags, final String category) {
        ...
        final Intent intent = new Intent()
                .setAction(action)
                .putExtras(toBundle(new Bundle(), extras));

        if (Integer.valueOf(flags) != null)
            intent.addFlags(flags);

        if (!category.equalsIgnoreCase(NULL))
            intent.addCategory(category);

        Log.v(TAG, String.format("Method %s", method));
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if (action.equals("fireNativeEvent")) {
            ...
            final String actionIntent = args.getString(0);
            if (actionIntent == null || actionIntent.isEmpty()) {
                callbackContext.error(EVENTNAME_ERROR);

            }
            final JSONObject extras = args.getJSONObject(1);

            final int flags = args.getInt(2);

            final String category = args.getString(3);


            cordova.getThreadPool().execute(new Runnable() {
                @Override
                public void run() {
                    sendGlobalBroadcast(actionIntent, extras, flags, category);
                }
            });

            callbackContext.success();
            return true;
        } else if (action.equals("registerExternalIntentReceiver")) {
            final String eventName = args.getString(0);
            if (eventName == null || eventName.isEmpty()) {
                callbackContext.error(EVENTNAME_ERROR);
                return false;
            ...
            if (!receiverMap.containsKey(eventName)) {
                ...
                registerExternalIntentReceiver(r, new IntentFilter(eventName));
            ...
            return true;
        } else if (action.equals("unregisterExternalIntentReceiver")) {
            ...
            if (r != null) {
                unregisterExternalIntentReceiver(r);
            }
            callbackContext.success();
            return true;
        }
